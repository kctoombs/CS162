...
case class MyCons[A](x: A, xs: MyList[A]) extends MyList[A] {
  ...
  //{
    //this match {
      //case MyCons(x, xs) => MyCons(f(x), xs.flatMap(f))
    //}
  //}

  def filter(pred: A => Boolean): MyList[A] = foldRight(MyList[A]()){
  ((value,acc) => if(pred(value)) MyCons(value, acc) else acc)
}	
  
  def append(other: MyList[A]): MyList[A] = if(other.isEmpty) this else MyCons(x, xs.append(other.take(other.length)))

  def foldLeft[B](initial: B)(f: (B, A) => B): B ={
    this match {
      case MyCons(x, xs) => {
        val current = f(initial, x)
        xs.foldLeft(current)(f)
      }
    }
  }

  def foldRight[B](initial: B)(f: (A, B) => B): B = if(this.isEmpty) initial else f(x,xs.foldRight(initial)(f))

  def take(n: Int): MyList[A] = if(n == 0) MyNil()
   else{
    if(n == 1) MyCons(x, MyNil()) else(MyCons(x, xs.take(n-1)))
}

  def drop(n: Int): MyList[A] = if(n <= 0) this else xs.drop(n - 1)

  def head: A = x

  def tail: MyList[A] = xs

  def init: MyList[A] = this.take(this.length - 1)

  def last: A = this.foldLeft[A](x)((_, y) => y)
  ...
  def length: Int = if(this.isEmpty) 0 else 1 + xs.length
...
case class MyNil[A]() extends MyList[A] {
  ...
  def map[B](f: A => B): MyList[B] = MyList[B]()

  def flatMap[B](f: A => MyList[B]): MyList[B] = MyList[B]()

  def filter(pred: A => Boolean): MyList[A] = MyList[A]()
  ...
  def take(n: Int): MyList[A] = MyList[A]()

  def drop(n: Int): MyList[A] = MyList[A]()

  def head: A = throw InvalidOperationException("head called on empty list")

  def tail: MyList[A] = throw InvalidOperationException("tail called on empty list")

  def init: MyList[A] = throw InvalidOperationException("init called on empty list")

  def last: A = throw InvalidOperationException("last called on empty list")

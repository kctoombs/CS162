...
object Problem3 {
  def main( args:Array[String] ) = {
    ...
    println(list.foldLeft("") ((word, cur) =>  
      if (cur > word) cur else word))
  
    println(list.foldLeft(List[String]()) ((accum,word) => if (accum.contains(word)) accum else word :: accum).reverse)

    println(list.foldLeft(Map[String, Int]()){
    (a,b) => a + (b->1 + a(b).toList)
    })

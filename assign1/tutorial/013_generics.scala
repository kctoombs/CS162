...
// an object to invoke its specialized code regardless of what we believe its
// static type to be.
//
// For example in:
// Shape s = new Circle();
...
def applySecond[T,U](list: MyList2[T], function: T => U): U = function((list.tail).head)//We want the head of the tail. head->[][]<-tail (or head of tail)

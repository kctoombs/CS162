...
def myMapUsingFlatMap[A, B](input: List[A], function: A => B): List[B] =
  myFlatMap(input, (item: A) => List[B](function(item)))
...
def myFilterUsingFlatMap[A](input: List[A], function: A => Boolean): List[A] =
  myFlatMap(input, (item: A) => if(function(item)) List[A](item) else List[A]())
...
assert(Seq(1, 2, 3).foldLeft(List[Int]())((b,a) => a :: b) == List(3, 2, 1)) // don't use reverse

// hint: Strings have a `length` method
assert(Seq("moo", "cow", "bull").foldRight(List[Int]())((a,acc) => a.length :: acc) == List(3, 3, 4))

...
object Images {
  ...
  // !!FILL ME IN
  def lift0[A](value: A): Img[A] = (p:Point) => value
  ...
  // !!FILL ME IN
  def lift1[A, B](f: A => B)(image: Img[A]): Img[B] = (p:Point) => f(image(p))
  ...
  // of type C.
  // !!FILL ME IN
  def lift2[A,B,C](f: (A,B) => C)(imageA: Img[A])(imageB: Img[B]): Img[C] =
  (p:Point) => f(imageA(p), imageB(p)) 
  ...
  // !!FILL ME IN
  def lift3[A,B,C,D](f: (A,B,C) => D)(imageA: Img[A])(imageB: Img[B])(imageC: Img[C]): Img[D] = (p:Point) => f(imageA(p), imageB(p), imageC(p))
  ...
  // colour images.
  // !!FILL ME IN
  val lerpImgClr: ImgGray => ImgClr => ImgClr => ImgClr = (i1: ImgGray) => (i2: ImgClr) => (i3: ImgClr) => lift3(lerpClr)(i1)(i2)(i3)
  ...
  // !!FILL ME IN
  val overlayImgClr: ImgClr => ImgClr => ImgClr = (i1: ImgClr) => (i2: ImgClr) => lift2(overlayClr)(i1)(i2)
  ...
  // ImgMask argument.
  // !!FILL ME IN
  val selectImgClr: ImgMask => ImgClr => ImgClr => ImgClr = (i1: ImgMask) => (i2: ImgClr) => (i3: ImgClr) => lift3((b: Boolean, c1: Colour, c2: Colour) => if(b) c1 else c2)(i1)(i2)(i3) 
  ...
  // !!FILL ME IN
  val msk2clr: ImgMask => ImgClr = (i: ImgMask) => lift1((b: Boolean) => if(b) black else white)(i)
  ...
  // !!FILL ME IN
  val gray2clr: ImgGray => ImgClr = (i: ImgGray) => lift1((v: Value) => Colour(v, v, v, 1))(i)
}
...
object SpatialTransforms {
  ...
  // !!FILL ME IN
  def rotatePt(angle: Double): TransformPt =(p:Point) =>
  Point((p.x*cos(angle) - p.y*sin(angle)),(p.y*cos(angle) + p.x*sin(angle)))
  ...
  // !!FILL ME IN
  def swirlPt(angle: Double): TransformPt = (p:Point) =>
  rotatePt(dist(p) * (2*scala.math.Pi/angle))(p) 

  // define a method named 'polarXf' that takes a TransformPlr as
  // argument and returns a TransformPt.
  // !!FILL ME IN
  def polarXf(value: TransformPlr): TransformPt = (p:Point) => plr2pt(value(pt2plr(p)))
  ...
  // !!FILL ME IN
  def translate[A](v: Vector): Filter[A] = (image: Img[A]) => (p:Point) => image(translatePt(v.neg)(p))
  ...
  // !!FILL ME IN
  def scale[A](v: Vector): Filter[A] = (i:Img[A]) => (p: Point) => i(scalePt(v.inverse)(p))
  ...
  // !!FILL ME IN
  def rotate[A](angle: Double): Filter[A] = (i:Img[A]) => (p: Point) => i(rotatePt(-1*angle)(p))
  ...
  // !!FILL ME IN
  def swirl[A](angle:Double): Filter[A] = (i:Img[A]) => (p: Point) => i(swirlPt(-1*angle)(p))
}
...
object Masking {
  ...
  // !!FILL ME IN
  val notM: ImgMask => ImgMask = (i: ImgMask) => lift1[Boolean, Boolean]((b: Boolean) => (if (b) false else true))(i)
  ...
  // !!FILL ME IN
  val intersectM: ImgMask => ImgMask => ImgMask = (i1: ImgMask) => (i2: ImgMask) => lift2[Boolean, Boolean, Boolean]((b1: Boolean, b2: Boolean) => if (b1 && b2) true else false)(i1)(i2)
  ...
  // !!FILL ME IN
  val unionM: ImgMask => ImgMask => ImgMask = (i1: ImgMask) => (i2: ImgMask) => lift2[Boolean, Boolean, Boolean]((b1: Boolean, b2: Boolean) => if (b1 || b2) true else false)(i1)(i2)
  ...
  // !!FILL ME IN
  val xorM: ImgMask => ImgMask => ImgMask = (i1: ImgMask) => (i2: ImgMask) => lift2[Boolean, Boolean, Boolean]((b1: Boolean, b2: Boolean) => if (b1 ^ b2) true else false)(i1)(i2)
  ...
  // !!FILL ME IN
  val diffM: ImgMask => ImgMask => ImgMask = (i1: ImgMask) => (i2: ImgMask) => lift2[Boolean, Boolean, Boolean]((b1: Boolean, b2: Boolean) => if (b1 && !b2) true else false)(i1)(i2)
}

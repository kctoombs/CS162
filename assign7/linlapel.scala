...
object Interpreter {
  ...
  def eval(goals: Seq[TODO], _Δ: Seq[Assume], eq: Equiv): Unit =
    if (goals nonEmpty) goals.head match {
      ...
      delta.foldLeft(0)((value, result) => {
    
      result match {

      case w @ With(list: Seq[Assume]) => list map{
           case Fact(values, obj) => {
      	        val delta : Seq[Assume] = _Δ
      	   	val t = goals.tail
      	   	val div = delta.splitAt(value)
		val div1 = div._1
      	   	val div2 = div._2
      	   	val gamma = _Γ
		...
		val vMap = values.map(_ -> LogicV())
      		...
      		val preds = matchPredicates(c, gc, eq)
      		preds match {
      		case Some(some: Equiv) => eval(t, div1 ++ div2.tail, some)
      		case None => Unit
      		}
      	}
	   case Clause(set: Set[Var], pred: Predicate, g: Goal) => {
		val gamma = _Γ
		val delta : Seq[Assume] = _Δ
		val t = goals.tail
		val div = delta.splitAt(value)
		val div1 = div._1
      	   	val div2 = div._2
		val vMap = set.map(_ -> LogicV())
		val logicMap = vMap.toMap
		val c: Closure = Closure(pred, logicMap, gamma)
		val preds = matchPredicates(c, gc, eq)
		preds match {
		case Some(some: Equiv) => {
		val clo: Closure = Closure(g, logicMap, gamma)
		eval(clo +: t, div1 ++ div2.tail, some)
		}
		case None => Unit
		}
		}
      ...
      case Fact(set: Set[Var], p: Predicate) => {
      	   val delta = _Δ
	   val gamma =  _Γ
	   val t = goals.tail
      	   val div = delta.splitAt(value)
	   ...
	   val vMap = set.map(_ -> LogicV())
	   val logicMap = vMap.toMap
	   val c: Closure = Closure(p, logicMap, gamma)
	   val preds = matchPredicates(c, gc, eq)
	   preds match {
	   case Some(some: Equiv) => eval(t, div1 ++ div2.tail, some)
	   case None => Unit
      ...
      case Clause(set: Set[Var], p: Predicate, g: Goal) => {
      	   val delta = _Δ
	   val gamma =  _Γ
	   val t = goals.tail
      	   val div = delta.splitAt(value)
	   ...
	   val preds = matchPredicates(c, gc, eq)
	   preds match {
	   case Some(some : Equiv) => eval(c +: t, div1 ++ div2.tail, some)
	   case None => Unit
	   }
      }
 ...
 case w @ With(list: Seq[Assume]) => list map{
           case Fact(values: Set[Var], obj: Predicate) => {
      	        val delta : Seq[Assume] = _Δ
      	   	val t = goals.tail
      	   	val gamma = _Γ
		val vMap = values.map(_ -> LogicV())
      		...
      		val preds = matchPredicates(c, gc, eq)
      		preds match {
      		case Some(some: Equiv) => eval(t, delta, some)
      		case None => Unit
      		}
      	}
	   case Clause(set: Set[Var], pred: Predicate, g: Goal) => {
		...
		case Some(some: Equiv) => {
		val clo: Closure = Closure(g, logicMap, gamma)
		eval(clo +: t, delta diff Seq(w), some)
		}
		case None => Unit
		}
		}
